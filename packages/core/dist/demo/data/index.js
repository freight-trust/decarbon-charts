...
export * from "./scatter";
export * from "./step";
import { createChartSandbox, createReactChartApp, createAngularChartApp, createVueChartApp, createVanillaChartApp } from "./create-codesandbox";
...
export var storybookDemoGroups = Tools.clone(allDemoGroups);
// in the demo page we want to show only demos with isDemoExample = true
export var demoGroups = Tools.clone(allDemoGroups).map(function (demoGroup) {
    demoGroup.demos = demoGroup.demos.filter(function (demo) { return demo.isDemoExample; });
    return demoGroup;
}).filter(function (demoGroup) { return demoGroup.demos.length; }); // remove demoGroup if it's children are all with isDemoExample = false
